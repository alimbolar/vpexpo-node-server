...
const ZOHO_CONSOLE = 'youandeye-main-console';
...
const getAccessToken = async () => {
  if (currentToken && currentToken.expirationDate > new Date()) return currentToken.token;
  ...
  try {
    ...
    const tokenResponse = await fetch(zohoTokenUrl, { method: 'POST', body: null });
    ...
    if (jsonResponse.access_token) {
      ...
      currentToken.expirationDate.setSeconds(currentToken.expirationDate.getSeconds() + jsonResponse.expires_in - 60); // 60 secondes security
...
const sendToZoho = async (uri) => {
  try {
    ...
    let doPagination = uri.indexOf('limit=') === -1 && uri.indexOf('from=') === -1;

    const options = await getFetchOptions();
    let from = 0;
    let url = process.env.ZOHO_CREATOR_URL + uri;
    url += ((url.indexOf('?') === -1) ? '?' : '&') + 'limit=' + CONFIG.ZOHO_PAGINATION_LIMIT;
    let zohoResult = await fetch(url, options);
    let result = await zohoResult.json();
    const value = result;
    while (doPagination && result.data && result.data.length === CONFIG.ZOHO_PAGINATION_LIMIT) { // We are reached the zoho pagination limit
      from += CONFIG.ZOHO_PAGINATION_LIMIT;
      zohoResult = await fetch(url + '&from=' + from, options);
      result = await zohoResult.json();
      if (result.code === 3000 && result.data) value.data = value.data.concat(result.data);
    }

    if (value.code === 3000) { // Save in cache
      cache[uri] = {
        value,
        expirationDate: new Date().getTime() + CONFIG.ZOHO_CACHE_DURATION_HOURS * 3600 * 1000
      };
    }
    return value;
  } catch (error) {
...
const getSetting = async (country, setting) => {
  const settings = await sendToZoho(`/${ZOHO_CONSOLE}/report/${country}_Store_Settings`);
...
module.exports = {
  sendToZoho, clearCache, getSetting, getUserRole, ZOHO_CONSOLE
}
